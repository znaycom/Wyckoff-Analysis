...
with content_col:
    ...
    df["days_since_recommend"] = df["recommend_date_dt"].apply(lambda d: (today - d).days if d is not None else pd.NA)
    ...
    st.dataframe(
        ...
        .map(
            lambda v: (
                ...
                else ("color: green;" if isinstance(v, str) and "-" in v else "")
            ),
            subset=["累计涨跌幅"],
        )
